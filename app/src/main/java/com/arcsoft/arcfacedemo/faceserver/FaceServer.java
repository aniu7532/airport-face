...
public class FaceServer {
    ...
    private static FaceEngine frEngine = null;
    ...
    public synchronized void init(Context context, OnInitFinishedCallback onInitFinishedCallback) {
        if (faceEngine == null && context != null) {
            ...
            int engineCode = faceEngine.init(context, DetectMode.ASF_DETECT_MODE_IMAGE,
                    DetectFaceOrientPriority.ASF_OP_ALL_OUT, 1,
                    FaceEngine.ASF_FACE_RECOGNITION | FaceEngine.ASF_FACE_DETECT | FaceEngine.ASF_MASK_DETECT);
            if (engineCode == ErrorInfo.MOK) {
                initFaceList(context, null, onInitFinishedCallback, false);
            } else {
                faceEngine = null;
                ALog.e("init: failed! code = " + engineCode);
            }
        }
     ...
     *
     * @param context                上下文对象
     * @param engine             指定FaceEngine
     * @param onInitFinishedCallback 加载完成的回调
     * @param recognize              是否处于人脸识别流程
     */
    public void initFaceList(final Context context, FaceEngine engine,
            final OnInitFinishedCallback onInitFinishedCallback, boolean recognize) {
        Disposable disposable = Observable.create((ObservableOnSubscribe<Integer>) emitter -> {
            if (recognize) {
                frEngine = engine;
                ALog.e("recognize frEngine" + frEngine);
                List<FaceEntity> faceEntityList = FaceDatabase.getInstance(context).faceDao().getAllFaces();
                registerFaceFeatureInfoListFromDb(engine, faceEntityList);
     ...
     * @return 是否注册成功
     */
    public boolean registerNv21(Context context, byte[] nv21, int width, int height, FacePreviewInfo faceInfo,
            String name, FaceEngine frEngine, FaceEngine registerFaceEngine) {
        if (registerFaceEngine == null || context == null || nv21 == null || width % 4 != 0
                || nv21.length != width * height * 3 / 2) {
            ALog.e("registerNv21: invalid params");
            return false;
        }
        ...
        synchronized (registerFaceEngine) {
            code = registerFaceEngine.extractFaceFeature(nv21, width, height, FaceEngine.CP_PAF_NV21,
                    faceInfo.getFaceInfoRgb(), ExtractType.REGISTER, MaskInfo.NOT_WORN, faceFeature);
        }
        if (code != ErrorInfo.MOK) {
            ALog.e("registerNv21: extractFaceFeature failed , code is " + code);
            return false;
        } else {
            /*
            ...
            Bitmap headBmp = getHeadImage(nv21, width, height, faceInfo.getFaceInfoRgb().getOrient(), cropRect,
     ...
     */
    public void registerFaceFeatureInfoListFromDb(FaceEngine faceEngine, List<FaceEntity> faceEntityList) {
        ...
        for (FaceEntity faceEntity : faceEntityList) {
            FaceFeatureInfo faceFeatureInfo =
                    new FaceFeatureInfo((int) faceEntity.getFaceId(), faceEntity.getFeatureData());
    ...
    private void registerFaceFeatureInfoFromDb(FaceEntity faceEntity, FaceEngine faceEngine) {
        if (faceEntity != null && faceEngine != null) {
            ...
            ALog.i("registerFaceFeature:" + res);
        }
    }
    ...
    private String getImageDir() {
        return ArcFaceApplication.getApplication().getApplicationContext().getExternalFilesDir(
                Environment.DIRECTORY_PICTURES) + File.separator + "faceDB" + File.separator + "registerFaces";
    }
     ...
     * @return
     */
    public FaceEntity registerJpeg(Context context, byte[] jpeg, String name, boolean recognize)
            throws RegisterFailedException {
        if (faceRegisterInfoList != null && faceRegisterInfoList.size() >= MAX_REGISTER_FACE_COUNT) {
            ALog.e("registerJpeg: registered face count limited " + faceRegisterInfoList.size());
            // 已达注册上限，超过该值会影响识别率
        ...
        return registerBgr24(context, imageData, bitmap.getWidth(), bitmap.getHeight(), name, recognize);
    }
    ...
    public FaceEntity registerBgr24(Context context, byte[] bgr24, int width, int height, String name,
            boolean recognize) {
        ALog.e("faceEngine:" + faceEngine + ", bgr24:" + bgr24 + ", width:" + width + ", height:" + height + ", name:"
                + name);
        if (faceEngine == null || context == null || bgr24 == null || width % 4 != 0
                || bgr24.length != width * height * 3) {
            ALog.e("registerBgr24:  invalid params");
            return null;
        }
        ...
        if (code == ErrorInfo.MOK && !faceInfoList.isEmpty()) {
            ...
            if (code == ErrorInfo.MOK) {
                ...
                if (!maskInfoList.isEmpty()) {
                    ...
                    if (isMask == MaskInfo.WORN) {
                         ...
                         * 注册照要求不戴口罩
                         */
                        ALog.e("registerBgr24: maskInfo is worn");
                        return null;
                    }
                    if (isMask == MaskInfo.UNKNOWN) {
                        ALog.e("registerBgr24: maskInfo is unknown");
                        return null;
                    }
                }
            } else {
                ALog.e("registerBgr24 mask process fail res=" + code);
                return null;
            }
            ...
            if (code == ErrorInfo.MOK) {
                ...
                if (cropRect == null) {
                    ALog.e("registerBgr24: cropRect is null");
                    return null;
                }
                cropRect.left &= ~3;
                cropRect.top &= ~3;
                ...
                long faceId = FaceDatabase.getInstance(context).faceDao().insert(faceEntity);
                faceEntity.setFaceId(faceId);
                ALog.e("faceId:" + faceId);
                faceRegisterInfoList.add(faceEntity);
                if (recognize) {
                    registerFaceFeatureInfoFromDb(faceEntity, frEngine);
                }
                ALog.e("faceRegisterInfoList:" + faceRegisterInfoList.size());
                return faceEntity;
            } else {
                ALog.e("registerBgr24: extract face feature failed, code is " + code);
                return null;
            }
        } else {
            ALog.e("registerBgr24: no face detected, code is " + code);
            return null;
        }
    }
    ...
    public CompareResult searchFaceFeature(FaceFeature faceFeature, FaceEngine faceEngine) {
        ...
        try {
            ...
            ALog.i("searchCost:" + (System.currentTimeMillis() - searchStart) + "ms");
            if (searchResult != null) {
                FaceFeatureInfo faceFeatureInfo = searchResult.getFaceFeatureInfo();
                FaceEntity faceEntity = FaceDatabase.getInstance(ArcFaceApplication.getApplication()).faceDao()
                        .queryByFaceId(faceFeatureInfo.getSearchId());
                ALog.e("faceRegisterInfoList:" + faceRegisterInfoList.size());
                ALog.d("faceFeatureInfo.getSearchId()：" + faceFeatureInfo.getSearchId());
                if (faceEntity != null) {
                    ALog.d("银川机场.人脸id: " + faceEntity.getFaceId() + "用户名称:" + faceEntity.getUserName());
                    return new CompareResult(faceEntity, searchResult.getMaxSimilar(), ErrorInfo.MOK,
                            System.currentTimeMillis() - start);
                }
                ALog.d("faceEntity null");
            }
        } catch (IllegalArgumentException exception) {
            ALog.i("exception:" + exception.getMessage());
        }
        return null;
    }
