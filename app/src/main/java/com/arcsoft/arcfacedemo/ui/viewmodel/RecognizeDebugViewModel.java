...
public class RecognizeDebugViewModel extends ViewModel implements RecognizeCallback, DebugInfoCallback {
    ...
    public void init() {
        ...
        // 填入在设置界面设置好的配置信息
        boolean enableLiveness = !ConfigUtil.getLivenessDetectType(context)
        ...
        RecognizeConfiguration configuration = new RecognizeConfiguration.Builder()
                ...
                .enableLiveness(enableLiveness).enableImageQuality(enableFaceQualityDetect)
        ...
        dumpExecutor = new ThreadPoolExecutor(1, 1, 0L, TimeUnit.MILLISECONDS, new LinkedBlockingQueue<>(), r -> {
        ...
        dumpExecutor.execute(() -> {
            ...
            getNoticeLiveData().postValue(
                    String.format("crashLogDir:\n%s\n\ndebugLogDir:\n%s", DebugInfoDumper.CRASH_LOG_DIR, dumpFileDir));
            ...
            FileUtil.saveDataToFile("\r\n\r\n".getBytes(), path, true);
            FileUtil.saveDataToFile(String.format("ftOrient:%s", ConfigUtil.getFtOrient(context).toString()).getBytes(),
                    path, true);
            FileUtil.saveDataToFile("\r\n".getBytes(), path, true);
            FileUtil.saveDataToFile(
                    String.format("recognizeScale:%d", ConfigUtil.getRecognizeScale(context)).getBytes(), path, true);
            FileUtil.saveDataToFile("\r\n".getBytes(), path, true);
            FileUtil.saveDataToFile(String
                    .format("livenessType=%s", livenessType == null ? "null" : livenessType.toString()).getBytes(),
                    path, true);
        });
    ...
    private void initFaceHelper(Camera.Size lastPreviewSize) {
        if (faceHelper == null || lastPreviewSize == null || lastPreviewSize.width != previewSize.width
            ...
            faceHelper = new DebugFaceHelper.Builder().ftEngine(ftEngine).frEngine(frEngine).flEngine(flEngine)
                    .frQueueSize(maxDetectFaceNum).flQueueSize(maxDetectFaceNum).previewSize(previewSize)
                    .recognizeCallback(this).recognizeConfiguration(recognizeConfiguration.getValue())
                    .trackedFaceCount(
    ...
    @Override
    public void onCompareFailed(byte[] nv21, String fileName, String recognizeFeatureName, String registerFeatureName,
            byte[] recognizeFaceFeature, FaceEntity faceEntity) {
        DebugInfoDumper.saveCompareFailedData(dumpFileDir, fileName, nv21, recognizeFeatureName, registerFeatureName,
                recognizeFaceFeature, faceEntity, dumpExecutor);
    }
