...
public class CameraConfigureActivity extends BaseActivity
        implements ViewTreeObserver.OnGlobalLayoutListener, IDualCameraFaceInfoTransformer {
    ...
    private void initView() {
        ...
        getSupportFragmentManager().beginTransaction()
                .replace(R.id.fl_camera_config_container, cameraConfigPreferenceFragment).commit();
        getSupportFragmentManager().beginTransaction()
                .replace(R.id.fl_dual_camera_offset_container, dualCameraOffsetPreferenceFragment).commit();
        getSupportFragmentManager().beginTransaction().replace(R.id.fl_preview_container, previewPreferenceFragment)
                .commit();
    }
    ...
    private void initRgbCamera() {
        CameraListener cameraListener = new CameraListener() {
            ...
            public void onCameraOpened(Camera camera, int cameraId, int displayOrientation, boolean isMirror) {
                ...
                ViewGroup.LayoutParams layoutParams = adjustPreviewViewSize(binding.textureViewRgb,
                        binding.faceRectViewRgb, binding.glSurfaceViewRgb, previewSizeRgb, displayOrientation);
                // GLSurfaceView以原画显示
                binding.glSurfaceViewRgb.init(false, 0, previewSizeRgb.width, previewSizeRgb.height);
                rgbRectTransformer = new FaceRectTransformer(previewSizeRgb.width, previewSizeRgb.height,
                        layoutParams.width, layoutParams.height, displayOrientation, cameraId, isMirror,
                        drawRgbRectHorizontalMirror, drawRgbRectVerticalMirror);
                ...
                if (previewSizeRgb.width > binding.glSurfaceViewRgb.getLayoutParams().width) {
                    glSurfaceRectStrokeWidth =
                            2 * previewSizeRgb.width / binding.glSurfaceViewRgb.getLayoutParams().width;
                }
            }
            ...
            public void onPreview(final byte[] nv21, Camera camera) {
                ...
                if (!facePreviewInfoList.isEmpty()) {
                    ...
                    rgbDrawInfoList.add(new FaceRectView.DrawInfo(facePreviewInfoList.get(0).getRgbTransformedRect(),
                            GenderInfo.UNKNOWN, AgeInfo.UNKNOWN_AGE, LivenessInfo.UNKNOWN, Color.GREEN, ""));
                } else {
        ...
        rgbCameraHelper = new DualCameraHelper.Builder()
                .previewViewSize(new Point(binding.textureViewRgb.getMeasuredWidth(),
                        binding.textureViewRgb.getMeasuredHeight()))
                .rotation(getWindowManager().getDefaultDisplay().getRotation())
                .additionalRotation(previewConfig.getRgbAdditionalDisplayOrientation())
                .specificCameraId(previewConfig.getRgbCameraId()).previewSize(specificPreviewSize)
                .isMirror(drawRgbPreviewHorizontalMirror).previewOn(binding.textureViewRgb)
    ...
    private void initIrCamera() {
        CameraListener irCameraListener = new CameraListener() {
            ...
            public void onCameraOpened(Camera camera, int cameraId, int displayOrientation, boolean isMirror) {
                ...
                ViewGroup.LayoutParams layoutParams = adjustPreviewViewSize(binding.textureViewIr,
                        binding.faceRectViewIr, binding.glSurfaceViewIr, previewSizeIr, displayOrientation);
                // GLSurfaceView以原画显示
                binding.glSurfaceViewIr.init(false, 0, previewSizeIr.width, previewSizeIr.height);
                irRectTransformer = new FaceRectTransformer(previewSizeIr.width, previewSizeIr.height,
                        layoutParams.width, layoutParams.height, displayOrientation, cameraId, isMirror,
                        drawIrRectHorizontalMirror, drawIrRectVerticalMirror);
                ...
                if (previewSizeIr.width > binding.glSurfaceViewIr.getLayoutParams().width) {
                    glSurfaceRectStrokeWidth =
                            2 * previewSizeIr.width / binding.glSurfaceViewIr.getLayoutParams().width;
                }
            }
            ...
            public void onPreview(final byte[] nv21, Camera camera) {
                ...
                if (facePreviewInfoList != null && !facePreviewInfoList.isEmpty()) {
                    ...
                    irDrawInfoList.add(new FaceRectView.DrawInfo(facePreviewInfoList.get(0).getIrTransformedRect(),
        ...
        irCameraHelper = new DualCameraHelper.Builder()
                .previewViewSize(
                        new Point(binding.textureViewIr.getMeasuredWidth(), binding.textureViewIr.getMeasuredHeight()))
                .rotation(getWindowManager().getDefaultDisplay().getRotation())
                .additionalRotation(previewConfig.getIrAdditionalDisplayOrientation())
                .specificCameraId(previewConfig.getIrCameraId()).previewOn(binding.textureViewIr)
                .cameraListener(irCameraListener).isMirror(drawIrPreviewHorizontalMirror)
                .previewSize(specificPreviewSize).build();
        irCameraHelper.init();
        try {
    ...
    private ViewGroup.LayoutParams adjustPreviewViewSize(View previewView, FaceRectView faceRectView,
            CameraGLSurfaceView glSurfaceView, Camera.Size previewSize, int displayOrientation) {
    ...
    protected void afterRequestPermission(int requestCode, boolean isAllGranted) {
        ...
        if (isAllGranted) {
            ...
            if (rgbCameraHelper != null || irCameraHelper != null) {
                ...
                cameraConfigPreferenceFragment
                        .notifyCommonPreviewSize(dualCameraHelper.getCommonSupportedPreviewSize());
                cameraConfigPreferenceFragment.notifyCurrentPreviewSize(previewSize);
            }
        } else {
    ...
    private void initEngine() {
        ...
        int initCode = ftEngine.init(this, DetectMode.ASF_DETECT_MODE_VIDEO, ConfigUtil.getFtOrient(this), 1,
                FaceEngine.ASF_FACE_DETECT);

        if (initCode != ErrorInfo.MOK) {
            String error = getString(R.string.specific_engine_init_failed, "ftEngine", initCode,
                    ErrorCodeUtil.arcFaceErrorCodeToFieldName(initCode));
    ...
    private void initFaceHelper(Camera.Size lastPreviewSize) {
        if (faceHelper == null || lastPreviewSize == null || lastPreviewSize.width != previewSize.width
            ...
            faceHelper = new FaceHelper.Builder().ftEngine(ftEngine).previewSize(previewSize).onlyDetectLiveness(true)
                    ...
                    .dualCameraFaceInfoTransformer(this).build();
        }
    }
    ...
    private void rotateRgbCamera(int rotation) {
        ...
        rotateCameraPreview(rotation, binding.textureViewRgb, binding.faceRectViewRgb, rgbRectTransformer,
                rgbCameraHelper, previewSize);
    }

    private void rotateIrCamera(int rotation) {
        previewConfig.setIrAdditionalDisplayOrientation(rotation);
        rotateCameraPreview(rotation, binding.textureViewIr, binding.faceRectViewIr, irRectTransformer, irCameraHelper,
                previewSize);
    }

    private void rotateCameraPreview(int rotation, View previewView, FaceRectView faceRectView,
            FaceRectTransformer faceRectTransformer, DualCameraHelper cameraHelper, Camera.Size previewSize) {
    ...
    public static class CameraPreferenceFragment extends PreferenceFragmentCompat
    ...
    public static class DualCameraOffsetPreferenceFragment extends PreferenceFragmentCompat
            implements Preference.OnPreferenceChangeListener {
    ...
    public static class PreviewPreferenceFragment extends PreferenceFragmentCompat
            implements Preference.OnPreferenceChangeListener {
