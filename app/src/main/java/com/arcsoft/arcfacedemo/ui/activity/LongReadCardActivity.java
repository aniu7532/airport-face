...
public class LongReadCardActivity extends AppCompatActivity {

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_long_read_card);
    ...
    private void runCmd(EC_API api) {
        ...
        /*
         * //不常用的命令示例 //读配置块3命令 if(api.EC_ReadCfgBlock(rf, (byte) 3)){ log.showSendReceiveData(rf);
         * log.Text(LOG_ADD,"读配置块3成功\n\n"); //记录配置块数据，保证写配置时不更改配置 data=new byte[8]; for(int k=0;k<8;k++) {
         * data[k]=rf.RecvData[k+6]; } //记录上传天线编号标识 if((data[0]>>>4&0x01)==1) { isUploadAntNum=true;//网络盘点解析天线口号用到 }
         * }else{ log.Text(LOG_ADD,"读配置块3失败\n"); return; }
         * 
         * //写配置块3命令 if(data!=null&&api.EC_WriteCfgBlock(rf,data, (byte) 3)){ log.showSendReceiveData(rf);
         * log.Text(LOG_ADD,"写配置块3成功\n\n"); data=null; }else{ log.Text(LOG_ADD,"写配置块3失败\n"); return; }
         * 
         * //保存配置块命令 if(api.EC_SaveCfgBlock(rf, (byte) 0)){ log.showSendReceiveData(rf);
         * log.Text(LOG_ADD,"保存配置块成功\n\n"); }else{ log.Text(LOG_ADD,"保存配置块失败\n"); return; }
         * 
         * //噪音检测命令 if(api.EC_NoiseCheck(rf,(byte)0)){ log.showSendReceiveData(rf); log.Text(LOG_ADD,"噪音检测成功\n\n");
         * }else{ log.Text(LOG_ADD,"噪音检测失败\n"); return; }
         * 
         * //打开射频命令 if(api.EC_Open_CloseRFPower(rf,(byte)1)){ log.showSendReceiveData(rf);
         * log.Text(LOG_ADD,"打开射频成功\n\n"); }else{ log.Text(LOG_ADD,"打开射频失败\n"); return; }
         * 
         * //关闭射频命令 if(api.EC_Open_CloseRFPower(rf,(byte)0)){ log.showSendReceiveData(rf);
         * log.Text(LOG_ADD,"关闭射频成功\n\n"); }else{ log.Text(LOG_ADD,"关闭射频失败\n"); return; }
         * 
         * //蜂鸣器闪烁命令 if(api.EC_HardwareControl(rf,(byte)1,(byte)0x80,(byte)0)){ log.showSendReceiveData(rf);
         * log.Text(LOG_ADD,"蜂鸣器闪烁成功\n\n"); }else{ log.Text(LOG_ADD,"蜂鸣器闪烁失败\n"); return; }
         * 
         * //继电器闪烁命令 if(api.EC_HardwareControl(rf,(byte)1,(byte)1,(byte)0)){ log.showSendReceiveData(rf);
         * log.Text(LOG_ADD,"继电器闪烁成功\n\n"); }else{ log.Text(LOG_ADD,"继电器闪烁失败\n"); return; }
         */
