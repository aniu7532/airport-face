...
public class FaceCompareActivity extends BaseActivity {
    ...
    private void initEngine() {
        ...
        faceEngineCode = mainFaceEngine.init(this, DetectMode.ASF_DETECT_MODE_IMAGE,
    ...
    public void processImage(Bitmap bitmap, int type) {
        ...
        if ((ageCode | genderCode | maskInfoCode) != ErrorInfo.MOK) {
            showToast("at lease one of age、gender、mask detect failed! codes are: " + ageCode + " ," + genderCode + " ,"
                    + maskInfoCode);
            return;
        }
        ...
        int qualityCode = mainFaceEngine.imageQualityDetect(bgr24, width, height, FaceEngine.CP_PAF_BGR24,
                faceInfoList.get(0), isMask, imageQualitySimilar);
        ...
        if (!faceInfoList.isEmpty()) {
            if (type == TYPE_MAIN) {
                ...
                int res = mainFaceEngine.extractFaceFeature(bgr24, width, height, FaceEngine.CP_PAF_BGR24,
                        faceInfoList.get(0), ExtractType.REGISTER, isMask, mainFeature);
                if (res != ErrorInfo.MOK) {
                    mainFeature = null;
                }
                Glide.with(ivMainImage.getContext()).load(bitmap).into(ivMainImage);
                StringBuilder stringBuilder = new StringBuilder();
                if (!faceInfoList.isEmpty()) {
                    stringBuilder.append("face info:\n\n");
                }
                for (int i = 0; i < faceInfoList.size(); i++) {
                    stringBuilder.append("face[").append(i).append("]:\n").append(faceInfoList.get(i)).append("\nage:")
                            .append(ageInfoList.get(i).getAge()).append("\ngender:")
                ...
                int res = mainFaceEngine.extractFaceFeature(bgr24, width, height, FaceEngine.CP_PAF_BGR24,
                        faceInfoList.get(0), ExtractType.RECOGNIZE, isMask, faceFeature);
                if (res == 0) {
                    ...
                    if (compareResult == ErrorInfo.MOK) {
                        ItemShowInfo showInfo = new ItemShowInfo(bitmap, ageInfoList.get(0).getAge(),
                                genderInfoList.get(0).getGender(), faceSimilar.getScore());
