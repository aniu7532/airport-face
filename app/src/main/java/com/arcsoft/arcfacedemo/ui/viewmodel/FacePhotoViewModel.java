...
public class FacePhotoViewModel extends ViewModel {
    ...
    public void updateOrInsertFaceEntity() {
    ...
    @SuppressLint("CheckResult")
    public void registerFromDecryptFile(Context context, File dir, BatchRegisterCallback callback) {
        if (!dir.exists()) {
            callback.onFinish(0, 0, 0, context.getString(R.string.please_put_photos, dir.getAbsolutePath()));
            return;
        }
        ...
        Observable.fromArray(files).flatMap((Function<File, ObservableSource<Boolean>>) file -> {
            try {
                // byte[] encryptedBytes = FileUtil.fileToData(file);
                // if (encryptedBytes == null) {
                // Log.e(TAG, "读取文件失败: " + file.getName());
                // failed[0]++;
                // return observer -> observer.onNext(true);
                // }
                //
                // // 对本地图片解密
                // byte[] decryptedBytes = ImageDownloader.decryptByte(encryptedBytes);
                ...
                FaceEntity faceEntity = faceRepository.registerJpeg(context, decryptedBytes, name);
                if (faceEntity == null) {
                    Log.e(TAG, "注册文件失败: " + file.getName());
                    failed[0]++;
                } else {
                    success[0]++;
                    increaseFaceCount();
                ...
                .subscribe(new Observer<Boolean>() {
                    ...
                    @Override
                    public void onError(Throwable e) {
                        Log.e(TAG, "Error in registerFromDecryptFile", e);
                        callback.onFinish(success[0], failed[0], total, e.getMessage());
                        disposable.dispose();
                    }
    ...
    public void init() {
        if (faceRepository == null) {
            ...
            instance.init(ArcFaceApplication.getApplication().getApplicationContext(),
                    faceCount -> initFinished.postValue(true));
