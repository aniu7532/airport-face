...
public class LoginActivity extends BaseActivity
    ...
    // }.getType();
    // Response<LongPassCards> longPassCardsResponse = gson.fromJson(response, responseType);
    // ALog.i("入参: " + gson.toJson(params));
    // ALog.d("更新通信证接口数据response: " + gson.toJson(longPassCardsResponse));
    //
    // if (longPassCardsResponse.getCode() == 200) {
    // LongPassCards longPassCards = longPassCardsResponse.getData();
    // if (longPassCards != null && longPassCards.getList() != null
    // && !longPassCards.getList().isEmpty()) {
    // // 下载图片到本地
    // ImageDownloader.downloadImages(longPassCards.list, LoginActivity.this);
    ...
    // }
    // } else {
    // ALog.d("更新通行证线程接口非200: " + longPassCardsResponse.getMsg());
    // updateNext = false;
    // handleUpdateComplete(longPassCardList);
    // }
    ...
    // ALog.i("更新到数据库的数据: " + gson.toJson(longPassCardList));
    // }
    ...
    // public String doInBackground() throws Throwable {
    // List<LongTermPass> longTermPassList = new ArrayList<>();
    // longPassCardList.forEach(longPassCard -> {
    // // ALog.d("接口数据longPassCard: "+gson.toJson(longPassCard));
    // LongTermPass longTermPass = Converters.convertToLongTermPass(longPassCard);
    // longTermPassList.add(longTermPass);
    // });
    ...
    // @Override
    // public String doInBackground() throws Throwable {
    // List<FaceEntity> faceEntityList = FaceDatabase.getInstance(LoginActivity.this).faceDao().getAllFaces();
    // longPassCardList.forEach(longPassCard -> {
    // faceEntityList.forEach(faceEntity -> {
    // if (faceEntity.getUserName().equals(longPassCard.cardId)) {
    // // 删除人脸
    // FaceDatabase.getInstance(LoginActivity.this).faceDao().deleteFace(faceEntity);
    // }
    // });
    // // 获取图片
    // Bitmap bitmap = ImageDownloader.loadAndDecryptImage(longPassCard.cardId, LoginActivity.this);
    // // 注册人脸
    // registerFaceByBitmap(bitmap);
    // });
    // return null;
    // }
    // });
