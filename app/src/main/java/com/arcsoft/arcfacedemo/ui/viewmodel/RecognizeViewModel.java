...
public class RecognizeViewModel extends ViewModel implements RecognizeCallback {
    ...
    private MutableLiveData<Integer> mainInitCode = new MutableLiveData<>();
    ...
    private void registerFace(final byte[] nv21, FacePreviewInfo facePreviewInfo) {
        ...
        registerNv21Disposable = Observable.create((ObservableOnSubscribe<Boolean>) emitter -> {
            ...
            int res = registerEngine.init(ArcFaceApplication.getApplication(), DetectMode.ASF_DETECT_MODE_IMAGE,
                    DetectFaceOrientPriority.ASF_OP_0_ONLY, 1,
                    FaceEngine.ASF_FACE_RECOGNITION | FaceEngine.ASF_IMAGEQUALITY);
            if (res == ErrorInfo.MOK) {
                // 判断no mask fq质量 过低过高则注册失败
                ImageQualitySimilar imageQualitySimilar = new ImageQualitySimilar();
                int qualityCode = registerEngine.imageQualityDetect(nv21.clone(), previewSize.width, previewSize.height,
                        FaceEngine.CP_PAF_NV21, facePreviewInfo.getFaceInfoRgb(), facePreviewInfo.getMask(),
                        imageQualitySimilar);
                ...
                float destQuality =
                        ConfigUtil.getImageQualityNoMaskRegisterThreshold(ArcFaceApplication.getApplication());
                if (quality < destQuality) {
                    Log.e(TAG, "image quality invalid");
                    registerEngine.unInit();
                    emitter.onNext(false);
                }
                boolean success = FaceServer.getInstance().registerNv21(ArcFaceApplication.getApplication(),
                        nv21.clone(), previewSize.width, previewSize.height, facePreviewInfo,
                        "registered_" + faceHelper.getTrackedFaceCount(), frEngine, registerEngine);
                registerEngine.unInit();
    ...
    public void init() {
        ...
        // 填入在设置界面设置好的配置信息
        boolean enableLive = !ConfigUtil.getLivenessDetectType(context)
                .equals(context.getString(R.string.value_liveness_type_disable));
        ...
        RecognizeConfiguration configuration = new RecognizeConfiguration.Builder()
                .enableFaceMoveLimit(enableFaceMoveLimit).enableFaceSizeLimit(enableFaceSizeLimit)
                .faceSizeLimit(ConfigUtil.getFaceSizeLimit(context)).faceMoveLimit(ConfigUtil.getFaceMoveLimit(context))
                .enableLiveness(enableLive).enableImageQuality(enableFaceQualityDetect)
        ...
        frInitCode.postValue(frEngine.init(context, DetectMode.ASF_DETECT_MODE_IMAGE,
                DetectFaceOrientPriority.ASF_OP_0_ONLY, 10, frEngineMask));

        mainFaceEngine = new FaceEngine();
        mainInitCode.postValue(mainFaceEngine.init(context, DetectMode.ASF_DETECT_MODE_IMAGE,
        ...
        if (enableLive) {
            ...
            int flEngineMask = (livenessType == LivenessType.RGB ? FaceEngine.ASF_LIVENESS
                    : (FaceEngine.ASF_IR_LIVENESS | FaceEngine.ASF_FACE_DETECT));
    ...
    public void destroy() {
        unInit();
        if (faceHelper != null) {
            ConfigUtil.setTrackedFaceCount(ArcFaceApplication.getApplication().getApplicationContext(),
    ...
    private void initFaceHelper(Camera.Size lastPreviewSize) {
        if (faceHelper == null || lastPreviewSize == null || lastPreviewSize.width != previewSize.width
            ...
            faceHelper = new FaceHelper.Builder().ftEngine(ftEngine).maskEngine(maskEngine).frEngine(frEngine)
                    .flEngine(flEngine).frQueueSize(maxDetectFaceNum).flQueueSize(maxDetectFaceNum)
                    .previewSize(previewSize).recognizeCallback(recognizeCallback)
            ...
            if (similarPass) {
                ...
                if (!isAdded) {
                    ...
                    if (compareResults != null) {
                        ...
                        getFaceItemEventMutableLiveData()
                                .postValue(new FaceItemEvent(compareResults.size() - 1, EventType.INSERTED));
                    }
                }
            }
        });
    ...
    public List<FaceRectView.DrawInfo> getDrawInfo(List<FacePreviewInfo> facePreviewInfoList, LivenessType livenessType,
        ...
        for (int i = 0; i < facePreviewInfoList.size(); i++) {
            ...
            drawInfoList.add(new FaceRectView.DrawInfo(
                    livenessType == LivenessType.RGB ? facePreviewInfoList.get(i).getRgbTransformedRect()
                    ...
                    facePreviewInfoList.get(i).getForeRect(),
                    facePreviewInfoList.get(i).getFaceInfoRgb().getFaceAttributeInfo(), drawRectInfo,
    ...
    public float getFeatureValue(Bitmap bitmap) {
        ...
        if (transformCode != ArcSoftImageUtilError.CODE_SUCCESS) {
            ALog.d("failed to transform bitmap to imageData, code is " + transformCode);
            return 0;
        }
        ...
        if (detectCode != 0 || faceInfoList.isEmpty()) {
            ALog.d("face detection finished, code is " + detectCode + ", face num is " + faceInfoList.size());
            return 0;
        }
        // 人脸质量检测
        ImageQualitySimilar imageQualitySimilar = new ImageQualitySimilar();
        ...
        if (qualityCode != ErrorInfo.MOK) {
            ALog.d("imageQualityDetect failed! code is " + qualityCode);
            return 0;
        }
        ALog.e("score:" + imageQualitySimilar.getScore());
