...
public class LivenessDetectActivity extends BaseActivity
    ...
    private boolean isLoopPaused = false;// 循环调用读卡
    ...
    private List<LongTermRecords> longTermRecords;
    ...
    private WeakHandler countdownHandler = new WeakHandler(new Handler.Callback() {
        ...
        public boolean handleMessage(@NonNull Message msg) {
            ...
            case 5:
                ...
                // ThreadUtils.executeByFixed(ArcFaceApplication.POOL_SIZE, new ThreadUtils.SimpleTask<Boolean>() {
                // @Override
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        button_set.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Intent intent = new Intent(LivenessDetectActivity.this, HomeActivity.class);
        ...
        binding.tvMore.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                new XPopup.Builder(LivenessDetectActivity.this)
                        .asCustom(new RecordsPopDialog(LivenessDetectActivity.this, direction)).show();
            }
        });
        ...
        binding.imgYuan.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (clickCount >= REQUIRED_CLICKS) {
                    // 达到目标后可以触发某项操作
                    clickCount = 0; // 重置计数
                    new XPopup.Builder(LivenessDetectActivity.this).isDestroyOnDismiss(true) // 对于只使用一次的弹窗，推荐设置这个
                            .popupPosition(PopupPosition.Left)// 右边
                            .hasStatusBarShadow(true) // 启用状态栏阴影
                            .asCustom(new CustomDrawerPopupView(LivenessDetectActivity.this)).show();
        ...
        // Intent intent = new Intent(LivenessDetectJinActivity.this, LoginActivity.class);
        ...
        if (typeDevice == 1) {
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    int portSate = BasicOper.dc_open("COM", LivenessDetectActivity.this, path, baudrate);
            ...
            // @Override
            // public String doInBackground() throws Throwable {
            // int portSate = BasicOper.dc_open("COM", LivenessDetectActivity.this, "/dev/ttyS3", 115200);
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    Card card = new AndroidSerialPort(LivenessDetectActivity.this);
        ...
        try {
            ...
            if (resultArr1[0].equals("0000")) {
                ...
                if (rfidLocal.equals(rfid)) {
                    ...
                    // ToastDialogManager.showCustomDialog(LivenessDetectActivity.this,2,"重复读卡");
                    return false;
                }
        ...
        ThreadUtils.executeByFixed(ArcFaceApplication.POOL_SIZE, new SmallTask() {
            ...
            public String doInBackground() throws Throwable {
                ...
                if (longTermPass != null) {
                    ...
                    // // ToastDialogManager.showCustomDialog(LivenessDetectActivity.this, 2, "证件过期");
                    // }
                    // });
                    // return null;
                    // }
                    ...
                    // // ToastDialogManager.showCustomDialog(LivenessDetectActivity.this, 2,
                    // // "证件已" + theCardIsExpired(longTermPass.status));
                    // }
                    // });
                    // return null;
                    // }
                    ...
                    runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            // ToastDialogManager.showCustomDialog(LivenessDetectActivity.this, 1, "请验证人脸");
                        }
                    });
                    // Bitmap bitmap =
                    // ImageDownloader.loadAndDecryptImage(longTermPass.id, LivenessDetectJinActivity.this);
                    ...
                    runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            // ToastDialogManager.showCustomDialog(LivenessDetectActivity.this, 2, "长期卡不存在");
                        }
                    });
                }
                return null;
        ...
        if (linshiID.equals("")) {
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    // ToastDialogManager.showCustomDialog(LivenessDetectActivity.this, 2, "临时卡请先刷长期卡");
                }
            });
        ...
        ThreadUtils.executeByFixed(ArcFaceApplication.POOL_SIZE, new SmallTask() {
            ...
            public String doInBackground() throws Throwable {
                ...
                if (longTermPass != null) {
                    ...
                    // // ToastDialogManager.showCustomDialog(LivenessDetectActivity.this, 2, "临时卡过期");
                    // }
                    // });
                    // return null;
                    // }
                    ...
                    if (leadingPeopleId == null && leadingPeopleId.length == 0) {
                        ...
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                showCustomDialog(2, "未关联长期卡");
                                // ToastDialogManager.showCustomDialog(LivenessDetectActivity.this, 2, "未关联长期卡");
                            }
                        });
                        return null;
                    }
                    ...
                    if (contains) {
                        ...
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                showCustomDialog(1, "临时卡请验证人脸");
                                // ToastDialogManager.showCustomDialog(LivenessDetectActivity.this, 1, "临时卡请验证人脸");
                            }
                        });
                    } else {
                        playAudio(mediaReject);
                        runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                showCustomDialog(2, "引领人请刷卡");
                                // ToastDialogManager.showCustomDialog(LivenessDetectActivity.this, 2, "引领人请刷卡");
                            }
                        });
                    }
                } else {
                    ...
                    runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            // ToastDialogManager.showCustomDialog(LivenessDetectActivity.this, 2, "临时卡不存在s");
                        }
                    });
                }
                return null;
    ...
    public void onFaceFeatureAvailable(Bitmap bitmap, float faceSimilar, float quality, boolean result) {
        ...
        if (result) {
            ...
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    // toast_verified_passed.setVisibility(View.VISIBLE);
                    showSuccessDialog();
                    // ToastDialogManager.showSuccess(LivenessDetectActivity.this);
            ...
            if (l > 3000) {
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        // 提示识别失败
                        // toast_verified_fail.setVisibility(View.VISIBLE);//页面更新
                        // Toast.makeText(LivenessDetectActivity.this, "识别失败", Toast.LENGTH_SHORT).show();
                        showFailedDialog();
                        // ToastDialogManager.showFailure(LivenessDetectActivity.this);
                        faceFeature = null;
                        isLoopPaused = false; // 接受识别结果,继续开始读卡
                        turnFragment1();// 切换成普通状态
                        if (faceSimilar > 0) {
    ...
    void check() {
        ...
        getRequest.execute(new JsonCallback<Base<Version>>() {
            ...
            public void onSuccess(Response<Base<Version>> response) {
                ...
                new XPopup.Builder(LivenessDetectActivity.this)
                        .asCustom(new UpdatePopDialog(LivenessDetectActivity.this, response.body().getData(),
