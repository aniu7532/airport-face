...
import static com.xuexiang.xupdate.entity.UpdateError.ERROR.PROMPT_CANCEL;
import static com.xuexiang.xupdate.entity.UpdateError.ERROR.PROMPT_IGNORE;
...
public class UpdateDialog extends BaseDialog implements View.OnClickListener, IDownloadEventHandler {
    public String TAG = "UpdateDialog";
    // ======顶部========//
    /**
     * 顶部图片
     */
     ...
     * @return 更新提示
     */
    public static UpdateDialog newInstance(@NonNull Context context, @NonNull UpdateEntity updateEntity,
            @NonNull IPrompterProxy prompterProxy, PromptEntity promptEntity) {
        UpdateDialog dialog = new UpdateDialog(context, promptEntity);
        dialog.setIPrompterProxy(prompterProxy).setUpdateEntity(updateEntity).setPromptEntity(promptEntity);
        dialog.initTheme(promptEntity.getThemeColor(), promptEntity.getFocusColor(), promptEntity.getTopResId(),
                promptEntity.getButtonTextColor(), promptEntity.getWidthRatio(), promptEntity.getHeightRatio());
        return dialog;
    }

    private UpdateDialog(Context context) {
        super(context, R.layout.xupdate_dialog_update);
        // mCurrentOrientation = ((Activity) context).getRequestedOrientation();
    }

    private UpdateDialog(Context context, PromptEntity promptEntity) {
        super(context, promptEntity.getLayout() > 0 ? promptEntity.getLayout() : R.layout.xupdate_dialog_update);
     ...
     * 初始化主题色
     */
    private void initTheme(@ColorInt int themeColor, @ColorInt int focusColor, @DrawableRes int topResId,
            @ColorInt int buttonTextColor, float widthRatio, float heightRatio) {
        if (themeColor == -1) {
            themeColor = ColorUtils.getColor(getContext(), R.color.xupdate_default_theme_color);
        }

        if (focusColor == -1) {
            focusColor = ColorUtils.getColor(getContext(), R.color.xupdate_default_focus_color);
        }
        ...
        setDialogTheme(themeColor, focusColor, topResId, buttonTextColor, widthRatio, heightRatio);
    }
     ...
     */
    private void setDialogTheme(int themeColor, int focusColor, int topResId, int buttonTextColor, float widthRatio,
            float heightRatio) {
        ...
        DrawableUtils.setBackgroundCompat(mBtnUpdate,
                DrawableUtils.getDrawable(UpdateUtils.dip2px(4, getContext()), focusColor, themeColor));
        DrawableUtils.setBackgroundCompat(mBtnBackgroundUpdate,
                DrawableUtils.getDrawable(UpdateUtils.dip2px(4, getContext()), focusColor, themeColor));
        ...
        DrawableUtils.setBackgroundCompat(mTvIgnore, DrawableUtils.getStrokeDrawable(
                UpdateUtils.dip2px(4, getContext()), UpdateUtils.dip2px(1, getContext()), focusColor));
    ...
    private void initWindow(float widthRatio, float heightRatio) {
        ...
        // UpdateLog.i(TAG + "， mCurrentOrientation:" + mCurrentOrientation);
        // UpdateLog.i(TAG + "，displayMetrics:" + displayMetrics.toString());
        // if (mCurrentOrientation == ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE) {
        // if (widthRatio > 0 && widthRatio < 1) {
        // lp.width = (int) (displayMetrics.heightPixels * widthRatio);
        // }
        // if (heightRatio > 0 && heightRatio < 1) {
        // lp.height = (int) (displayMetrics.widthPixels * heightRatio);
        // }
        // } else if (mCurrentOrientation == ActivityInfo.SCREEN_ORIENTATION_PORTRAIT) {
        // if (widthRatio > 0 && widthRatio < 1) {
        ...
        if (i == R.id.btn_update) {
            ...
            if (!UpdateUtils.isPrivateApkCacheDir(mUpdateEntity) && flag != PackageManager.PERMISSION_GRANTED) {
                ActivityCompat.requestPermissions((Activity) getContext(),
                        new String[] { Manifest.permission.WRITE_EXTERNAL_STORAGE }, REQUEST_CODE_REQUEST_PERMISSIONS);
            } else {
                installApp();
            }
            ...
            _XUpdate.onUpdateError(PROMPT_CANCEL, mUpdateEntity);
            dismiss();
        } else if (i == R.id.tv_ignore) {
            // 点击忽略按钮
            UpdateUtils.saveIgnoreVersion(getContext(), mUpdateEntity.getVersionName());
            _XUpdate.onUpdateError(PROMPT_IGNORE, mUpdateEntity);
            dismiss();
        }
    }
    ...
    private void onInstallApk() {
        _XUpdate.startInstallApk(getContext(), UpdateUtils.getApkFileByUpdateEntity(mUpdateEntity),
                mUpdateEntity.getDownLoadEntity());
    }
