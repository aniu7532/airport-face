...
public class DownloadService extends Service {
    ...
    private void stop(String contentText) {
        if (mBuilder != null) {
            mBuilder.setContentTitle(UpdateUtils.getAppName(DownloadService.this)).setContentText(contentText);
    ...
    public class DownloadBinder extends Binder {
        ...
        public void start(@NonNull UpdateEntity updateEntity, @Nullable OnFileDownloadListener downloadListener) {
            ...
            startDownload(updateEntity,
                    mFileDownloadCallBack = new FileDownloadCallBack(updateEntity, downloadListener));
        }
    ...
    private class FileDownloadCallBack implements IUpdateHttpService.DownloadCallback {
            ...
            if (canRefreshProgress(rate)) {
                ...
                if (mBuilder != null) {
                    mBuilder.setContentTitle(
                            getString(R.string.xupdate_lab_downloading) + UpdateUtils.getAppName(DownloadService.this))
        ...
        private boolean canRefreshProgress(int newRate) {
            if (mBuilder != null) {
                ...
                // notify方法会将Notification加入系统的通知队列，当前应用发出的Notification数量超过50时，不再继续向系统的通知队列添加Notification，即造成了notificationManagerCompat.notify(TAG,
                // NOTIFY_ID, notify)无效的现象。
    ...
    private void showDownloadCompleteNotification(File file) {
        ...
        PendingIntent contentIntent =
                PendingIntent.getActivity(DownloadService.this, 0, installAppIntent, PendingIntent.FLAG_UPDATE_CURRENT);
        if (mBuilder == null) {
            mBuilder = getNotificationBuilder();
        }
        mBuilder.setContentIntent(contentIntent).setContentTitle(UpdateUtils.getAppName(DownloadService.this))
                .setContentText(getString(R.string.xupdate_download_complete)).setProgress(0, 0, false)
                // .setAutoCancel(true)
